 ...
 * Description: Magic numbers are separated by spaces.
 */
void elf_print_magic(unsigned char *e_ident)
{
	int i;
...
/**
 * elf_print_class - Prints the class of an ELF header.
 * @e_ident: A pointer to an array containing the ELF class.
 */
void elf_print_class(unsigned char *e_ident)
{
	printf(" Class: ");
...
void print_type(unsigned int e_type, unsigned char *e_ident)
{
	...
	else if (e_ident[EI_DATA] == ELFDATA2MSB)
	{
		if ((e_type >> 8) == ET_REL)
			printf(" Type: REL (Relocatable file)\n");
		else if ((e_type >> 8) == ET_EXEC)
			printf(" Type: EXEC (Executable file)\n");
		else if ((e_type >> 8) == ET_DYN)
			printf(" Type: DYN (Shared object file)\n");
		else if ((e_type >> 8) == ET_CORE)
			printf(" Type: CORE (Core file)\n");
		else
			printf(" Type: <unknown: %x>\n", e_type);
	}
